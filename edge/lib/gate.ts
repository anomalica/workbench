...
export async function startSession(
  ...
  const a = await Promise.all(
    sample.map((c) => answerHmac(secret, hash, c.answer)),
  );
  const token = await signToken(
    secret,
    { h: hash, t: nowSec, a } as SessionPayload,
  );
  return {
    token,
    challenges: sample.map((c, i) => ({
      id: i + 1,
      before: c.before,
      after: c.after,
    })),
...
export async function scoreSession(
  ...
  if (!payload || !Array.isArray(payload.a)) {
  ...
  for (let i = 0; i < payload.a.length; i++) {
    ...
    if (given && (await answerHmac(secret, hash, given)) === payload.a[i]) {
      score++;
    }
  }

...
<div class="flex-1 overflow-y-auto">
  <div class="mx-auto max-w-6xl px-6 py-6">
    ...
    <p class="mt-1 max-w-prose text-sm text-on-surface-muted">
      What earns a page. Proposed topics come from the graph and carry the evidence
      behind them; seeded topics are ones you name, which then fill up as material
      arrives. Open any topic to read the brief a page would be written from.
    </p>
    ...
    <div class="mt-6 flex flex-wrap items-center gap-3">
      <h3 class="font-ui text-sm text-on-surface">
        {topics.length} proposed by the graph
      </h3>
      <div class="flex gap-1 text-xs">
        {#each [["all", `All ${topics.length}`], ["single", `Single-source ${singleCount}`], ["nobrief", "No brief"]] as [key, label]}
          <button
